...
public class BitcaskService implements IBitcask {
  ...
  String bitcaskDir =
      System.getenv().getOrDefault("BITCASK_DIR", "/home/omar/Documents/bitcaskDB/");
  private final Bitcask bitcask;

  public BitcaskService() {
    bitcask = BitcaskFactory.getInstance(bitcaskDir, SyncConfig.SYNC_EVERY_SEC);
  ...
  public void process(WeatherMessage msg) {
    try {
      ...
      log.error("Failed to write to Bitcask for station {}: {}", msg.station_id, e.getMessage());
    }
  }
